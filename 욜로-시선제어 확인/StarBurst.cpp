...
void StarBurst::ApplyRANSAC(void) {
	...
	/*if (*inliers_index == NULL) {
		printf("Ransac 실패 ---------\n");
		info.PUPIL.width = 50;
		info.PUPIL.height = 50;
		info.PUPIL.center_of_x = (rect1.tl().x + rect1.br().x) / 2;
		info.PUPIL.center_of_y = (rect1.tl().y + rect1.br().y) / 2;
		info.PUPIL.theta = 0;
	}
...
void StarBurst::DetectCoutourPoints(void) {
	...
	// 시작점을 각막반사점으로
	// int guess_crx = info.CORNEAL_REFLEX.center_of_x;
	// int guess_cry = info.CORNEAL_REFLEX.center_of_y;
	//printf("초깃 설정 좌표 (%d %d)", guess_crx, guess_cry);
